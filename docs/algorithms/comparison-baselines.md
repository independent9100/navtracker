...
## Cl-4 — One deployable PMBM configuration (universal across geographies)

> **STATUS — CLOSED 2026-07-12 (ADR-0003, honest claim, not a clean sweep).**
> The one deployable config is **`imm_cv_ct_pmbm_coverage_land_ivgate`** with the
> near-shore no-birth strip narrowed to **`offshore_halfwidth_m = 25 m`** (floor
> 0.10, per-config; ADR-0003, branch `cl4-adoption`). This closes the blocker
> below: env-2 (the shore-hugging channel) goes from **0/8 collapse → 8/8**, GOSPA
...
>
> **Dated deviations recorded on the card (NOT a clean 10%-everywhere sweep):**
> (i) philos-regime cluttered harbors gain **~+10.45 phantom tracks/scan, in-strip**
> (philos `card_err` +6.9 → +17.35) — the user re-priced this acceptable
> 2026-07-11/12 (operator-supervised near-land waters; an invisible real *mover*
> is the worse failure by ADR-0002). (ii) A **residual 0–25 m blind band**: a
> vessel that stays within 25 m of shore its whole track still never initiates —
> the parked **pending band** (ADR-0003) is the priced escalation if a deployment
> operates in that band.

**The claim (user-set north star, 2026-07-10).** There is ONE PMBM
configuration a consumer can wire and deploy that is good EVERYWHERE we
bench — open water, coastal, urban shore-hugging channel, harbor — without
per-geography config switching. "Good" is defined against the promotion
dossier's gauntlet (`docs/baselines/2026-07-10_pmbm_promotion_dossier.md`):

- On every gauntlet workload, the unified config matches the per-geography
  best PMBM variant within a stated tolerance (target: within 10% GOSPA,
  no workload collapse, lifetime/breaks within the #25-post-gate envelope).
- No geography ceiling: specifically, it must TRACK in the shore-hugging
  channel (Cl-1 env-2) where today's coverage stack goes to zero.
- Realtime holds (worst-scan < 148 ms with margin, per the perf-arc method).
- The innovation gate (#25) and the anchored-vessel veto ride along at
  their measured settings.

**Where we stand (dossier, 2026-07-10).** The family is already there in
pieces: `imm_cv_ct_pmbm_coverage_land_ivgate` is the coastal/open-water
best (Cl-1 env-1 17.44, CPA loss 6 s, realtime) but COLLAPSES in the
channel (documented <50 m offshore no-birth zone, ADR 0001); plain
`imm_cv_ct_pmbm_land` survives the channel (env-2 17.74) but lacks the
coverage floor's clutter discipline. The gap between them IS the claim.

**Known blocker + candidate paths (ranked; (a) measured 2026-07-11).** The
no-birth zone: (a) ADR 0001 A3 sensor-aware suppression — **NO-BUILD,
Phase-1 probe 2026-07-11** (merged 93c31ec; write-up
`docs/baselines/2026-07-11_cl4_phase1_a3_probe.md`). Revival evidence is
real (env-2 targets 100% in-band with abundant lidar/camera evidence), but
the guard cannot be certified: the revival sensors (lidar/camera) are
entirely absent from the philos guard workload, AND the camera is measured
to be a clutter source near shore (79–85% of in-band camera bearings are
off-target = structure, gate-robust — the ADR's warning, now quantified).
Lidar is a clean discriminator (<21% off-target) but Trondheim-only, so it
cannot carry a *one-config* claim. Durable: any future near-shore camera
use must be corroboration-gated, never a standalone exemption; A3-via-lidar
is a sensor-fit question for a lidar-equipped deployment, not a build on
this gauntlet. (b) a **conditional coverage floor (re-detection-based,
ADR 0001 A2 family)** — the plain `(chain ≥ M in N scans) AND (net
displacement ≥ D)` rule is **NO-BUILD, Phase-1b probe 2026-07-11** (merged
d5e68e8; `docs/baselines/2026-07-11_cl4_phase1b_floor_probe.md`): K1 needs
...
dead; re-measure only if Phase 1c fails) — or, last resort, write the
per-geography residual honestly into the claim.
Whatever the path: measured against the FULL dossier gauntlet (the
2026-06-30 lesson — the naive fix traded philos for the channel; the claim
requires BOTH).

**Why this is a claim and not a chore.** Per-geography config switching
pushes a tracking-domain decision onto the consumer/middleware and creates
a mis-configuration failure mode at exactly the boundary (harbor exit /
approach) where the picture matters most. One config = no seam.
...
| Clutter/birth-model campaign (the increment-8 redirect item 2) | Cl-3 | **CLOSED 2026-07-07 — measured negative at the birth channel + contract-boundary finding.** Design-first campaign with a **binding entry probe** (branch `clutter-birth-campaign`; ticket + design note dated 2026-07-06; eval-log 2026-07-07). **Durable results:** (1) the **λ_C-cancellation INVARIANT** — under `birth_existence_target` (all deployed configs) birth existence is λ_C-invariant (`r_new=r*`), so a spatial/per-sensor/count λ_C is inert on the deployed config; clutter levers must ride **`birthScale`, not λ_C** (root cause of the "cmap inert under PMBM" puzzle; named in `pmbm-design.md §3.2.2`). (2) the **channel-reach wall** — `birthScale` gates only births and cannot reach a *confirmed* Bernoulli; the §5.0 probe (`test_clutter_burst_birth_confirm_probe.cpp`, permanent regression test) showed `sim_ms_clutter_burst` phantoms confirm in **one scan** (confirmed 2→15) and persist, so a *perfect* birth fix floors `card_err` at **~2.69 > MHT +2.51** — birth-side suppression is structurally insufficient. (3) **contract-boundary finding** — the burst that dominates the metric is a *pre-extraction* duplicate-cloud shape owned by the **upstream extractor** (2026-07-06 ruling); the discriminator conflated an upstream problem with the small tracker-level residual (over-dispersed diffuse background). Both deep pivots (existence-side penalty; NB-in-update) **parked with triggers** (backlog #23): revisit only if post-water real data shows confirmed-phantom persistence in concentrated clutter upstream can't fix. **Redirect closed both ways** (suppression-side = increment 8; birth-side = this campaign); residual dense-harbor over-count → upstream extraction + water test. Phase 0 (backlog #21 band guard) shipped to master (`fb7782c`). | done (measured negative) | **CLOSED — durable: λ_C-invariant, channel map, minefield analysis, §5.0 regression probe** |
...
| PMBM runtime probe + Murty K=1 fix | Cl-3 (dev velocity + deployment realtime gate) | Probe (merged d90d979): 85% of PMBM wall = Hungarian solves from murtyKBest child generation AFTER the K-th accepted assignment (~82 wasted LSAP solves + full cost-matrix copies per scan at K=1, ~98.8% of all solves); cost-matrix construction ~2.7%, occupancy <0.1%; config-knob frontier FLAT (only r_min=1e-2 moves, −6%, byte-identical accuracy incl. gate suite); bench harness self-taxes ~10.5% (own GOSPA scoring). Fix (45a504d): one guarded break after the K-th accepted assignment — bit-identical at every K (existing tests pin K=1≡Hungarian + full K=6 enumeration). Measured: 515 s → 41.6 s (12.4×) on decimated kattwyk_08 285 s, accuracy identical to 6 decimals; decimated feed now ~6.8× FASTER than realtime → realtime gate (≥5× at 60–100 plots/scan) MET on this workload. Follow-ups open non-blocking (re-profile first — old profile obsolete): sparse/gated LSAP; --fast-metrics bench stride. RAW-density check 2026-07-06: undecimated 169-plots/scan window = 141 s / 285 s → 2.0× faster than realtime (was 20× slower) but FAILS the ≥5× margin gate → front-end extraction stays deployment-MANDATORY (margin-short, not hopeless); extraction stage needs its own realtime budget; per-scan max/p99 latency still unmeasured (both regimes). ROUND 2 (2026-07-06, merged fa0ca25): fresh profile post-fix = no dominant safe-class lever left (IMM per-mode measurement update spread across Eigen ops; murtyKBest down to ~2k calls/a few %); shipped per-scan latency cols (scan_proc_ms mean/p95/p99/max — determinism guard excludes wall-derived rows) + --fast-metrics (raw −10.3%, default-off byte-identical). PER-SCAN ANSWER: max scan 41 ms (dec) / 102 ms (raw) vs 148 ms interval — the tracker keeps up scan-by-scan even at raw density (1.45× worst-case margin). ARBITER RULING on the gate nuance: extraction stays deployment-MANDATORY — 1.45× worst-case on a quiet dev box does not clear the margin the 5× gate exists to provide (weaker deployment hardware, CPU sharing, density spikes), and raw density doubles the phantom count regardless of speed. The per-scan result is recorded as a RESILIENCE property: if the extraction stage fails live, the tracker degrades (thin margin + over-count) instead of collapsing. Priced findings parked (perf off the critical path): Position2D H-elision (bit-identical ~5-8%, blast radius all estimators — own ticket if ever needed); coarse gate prefilter (result-affecting); sparse LSAP deprioritised. SCOPE CLARIFICATION (2026-07-06, user ruling): 'extraction stays deployment-mandatory' means the PIPELINE needs plot-level input, NOT that navtracker builds the extractor — deployments receive tracks/plots from the radar API, a commercial extractor, or a separate application (all ingest paths already exist: TTM/RemoteTrack/Measurement). The bench Python clustering is research-data tooling only. In-house detections→plots extraction is PARKED with triggers: (a) a deployment with raw detections and no upstream extractor, or (b) measured evidence that fusion-informed extraction (track/AIS/camera context) beats upstream extractors on accuracy. Refinement (user, same day): clustering as TOOLING is encouraged wherever it adds value — Python until measured pain; a C++ version, if ever, lives in a NON-DELIVERED location (tools/ or a separate CMake project), never in navtracker_core/navtracker_nmea. ROUND 3 (2026-07-06, merged 5f55fab): mechanical sympathy on the IMM hot path (GaussianScore.hpp one-decomposition helper + 2x2 stack kernels + state-path single-decomposition): decimated 1.50x (42.3→28.3 s), raw 1.66x (154→92.9 s), raw worst-scan 137.9→76.7 ms (1.93x interval margin); philos KEEP byte-identical, flips isolated to the known harbor/pmbm_adapt_k3 knife-edge (backlog #21); T2 hoist blocked by design (no IEstimator perf hook — pluggable-hot-path invariant). Perf arc CLOSED again; campaign-replay goal met; 57 s margin stays upstream. docs/baselines/2026-07-05_pmbm_runtime_frontier.{md,csv} + eval-log 2026-07-05/06. | probe 1 day + fix same-day | **closed 2026-07-06 (fix shipped, gate met)** |
| **Cl-4: one deployable PMBM configuration** | Cl-4 | User-set north star 2026-07-10 (see the Cl-4 section). Unify the family: coverage stack (open-water best, channel-blind) + `pmbm_land` (channel-capable) into ONE config with no geography collapse, realtime, gate+veto riding along. Known blocker = the <50 m no-birth zone; ranked paths (a) ADR 0001 A3 sensor-aware suppression — **measured NO-BUILD 2026-07-11** (Phase-1 probe, merged 93c31ec: revival evidence real on env-2, but the guard is unscoreable cross-workload — env-2 has lidar/camera and no AIS, philos has neither lidar nor camera; camera measured ~80% off-target near shore = a clutter source, not a discriminator; lidar clean (<21%) but Trondheim-only; write-up `docs/baselines/2026-07-11_cl4_phase1_a3_probe.md`), (b) conditional coverage floor — plain (M,N,D) rule **measured NO-BUILD 2026-07-11** (Phase-1b, merged d5e68e8: K1 needs D≤72.6 m, pier-walk K3 needs D>84 m — empty feasible set; philos not binding at 0.2%), **Phase 1c now primary** (smoothness term — pier steps 16–20 m/s vs vessel ≤8 m/s — plus real-association displacement; the NN chainer was a conservative K3 proxy), (c) re-priced lowered gate (likely dead, measured 2026-06-30) or documented per-geography residual as last resort. Judged on the FULL promotion-dossier gauntlet. Pre-req: harbor truth-sort reconciliation (yardstick integrity) — **DONE 2026-07-10** (merged ad5781e): truth-sort fix was already fully landed (3ee491f+3aa9c58, ancestors of the dossier base); harbor truth verified clean (5 objects, 40 complete groups); `card_err +11.64` is a REAL uncharted-pier over-count, NOT a harness artifact — the harbor row is a trustworthy Cl-4 target, and driving it down (charted-pier A/B shows 11.64→7.43 headroom) is part of the claim. Write-up: `docs/baselines/2026-07-10_harbor_truthsort_reconcile.md`. **CLOSED 2026-07-12 (ADR-0003, option (a)):** ship `offshore_halfwidth_m=25 m` (floor 0.10, per-config) on `imm_cv_ct_pmbm_coverage_land_ivgate` — env-2 0/8→**8/8** GOSPA 13.38, env-1/harbor/Imazu/sim byte-identical, at two dated deviations (philos +6.9→+17.35 in-strip phantoms, re-priced acceptable; 0–25 m residual blind band, pending band parked). The revive-near-structure arc (A3 / kinematic floor / occupancy-veto / shape) was exhausted first — all measured NO-BUILD; the strip-narrowing + honest deviation was the endgame. Gauntlet freeze: eval-log 2026-07-12. | probe ~2 days; build 1-2 wks | **CLOSED 2026-07-12 (ADR-0003) — honest claim, not a clean sweep** |
| Pre-water window (interim selection while the real test waits) | Cl-3 tail + deployment readiness | Research arc closed (corroboration complete, steady-state measured, detector frozen at 0.6, held-out scored 3H/1M/4P); water test delayed → one SELECTION doc picks from the existing queues (no new tracking system): Tier 1 = R11 SHIPPED 2026-07-05 (54c20cb: platform_id on TrackAttributes/output; PMBM refreshAggregatedTracks adopts identity from highest-mass carrying hypothesis, persisted on the Bernoulli last-write-wins so ids survive feed drops; fusion test surfaces both mmsi+platform_id; closes R9 item 3 too) + backlog #20 target-reported kinematics (AIS nav-status = the veto's missing data path) + #17 bearing-wedge + #18 fact-free half + #16; Tier 2 = D2 Stone Soup cross-check (never done), D7 MOANA (non-AIS truth + Doppler-capable radar), D8 R-BAD, R8.8 re-extraction (still open), ais_ferry_far/almost_cross measurement pass (held-out duty discharged; ais_ferry_far = only philos clip with honest accuracy truth); Tier 3 = at most ONE of clutter-model investigation / Cl-1 cold-start gap / D10+D3. Doc: `docs/superpowers/plans/2026-07-05-pre-water-window.md`. | tiers 1+2 ≈ 1 wk | **open — Tier 1 nearly done 2026-07-06: #20 (3 increments, anchored-veto data path live), #17 (BearingWedgeModel), #18 fact-free half all SHIPPED; #16 remains; Tier 2: D2 Stone Soup DONE 2026-07-06 (b5b3ea5 — GOSPA kernel == Stone Soup to 1.4e-14 on sim+real, prior incidents were upstream truth-grouping, kernel externally validated); #16 SHIPPED 2026-07-06 (a3528af — Tier 1 COMPLETE); item-10 philos farcross pass DONE 2026-07-06 (09edb64: PMBM ~46-54 m radar-only vs AIS truth 19 s spot check — the one honest pre-water real-data accuracy number; radartruth exposed as AIS-in-radar-frame, relabeled, independence audit clean); TIER 2 CLOSED 2026-07-06: R8.8 fully done (occlusion pass — shadow interval 50-85 s measured, R4 UNKNOWN = moored yachts, labels e716373); D8 GO (mmWave regime caveat, extraction queued behind Tier-3); D7 NO-GO (CC-BY-NC-SA license fails + no Doppler in published data — Doppler prototyping now waits on deployment hardware facts). Tier-3 CLOSED 2026-07-07 (clutter/birth: measured negative at birth channel, zero waste — see row below). D8 EXTRACTION SHIPPED 2026-07-08 (487e747, verified in isolation 1085/1085): 6 berthing approaches replayable, cross-tracker-consistency grade — both trackers hold berthing targets (breaks ~0); navtracker confirms 130 ids where the reference tracker minted 235 (OPPOSITE polarity to philos over-count — confirmation discipline is conservative on mmWave clutter); doppler+snr columns carried for a future Doppler probe. Remaining = deployment-facts emails (user) — the board is otherwise CLEAR (2026-07-08 user ruling: the ingestion middleware/main is the user's own system; navtracker's contract ends at Measurement/OwnShipPose — no middleware-side work item exists); MULTI-SENSOR SIM GATES SHIPPED 2026-07-06 (3d9543a: 6 seeded scenarios, independent truth by construction, first fusion accuracy gate — fusion ~2x radar-only on dropout continuity; compound-K clutter_burst = the designed lambda_C discriminator for the Tier-3 campaign)** |
| Data-expansion TODOs D1–D12 (unused `data/` sources + external sweep) | Cl-3 + measurement integrity | Bench today wires only philos + autoferry + one HAXR station-hour; ~2.3 GB of fetched data unused. D1 HAXR multi-station/hour (second real harbor cross-check for Stage 1b; also hosts increment-8 steady-state A/B + AIS-veto validation since labelled philos clips are zero-AIS), D2 Stone Soup GOSPA cross-validation (hedge after two truth-fragmentation harness bugs), D3 MarineCadastre/DMA anchored-vessel mining (R3/1b-ii corroboration input) + density soak, D4 Pohang moving-platform occupancy (parked for Stage 2), D5 SMD EO bearings (parked for ADR 0001 A3), D6 Kystverket soak (unscheduled). External sweep 2026-07-03 (evaluate-before-extract): **D7 MOANA** (real X-band radar with camera/lidar — NON-AIS — truth + anchored large vessels; first real source where "false track" means false), **D8 R-BAD** (69 h berthing radar + synced video: hour-scale steady state + philos-style label passes on a second geography), D9 DLR extended-target overlap check (~1 h), D10 GFW anchorage DB (pre-computed D3 stats + candidate KEEP-side chart-corroboration prior), D11 Reeds (Stage-2 moving-platform, evaluate before D4), D12 WaterScenes (logic cross-check only, wrong radar physics). Tickets: `docs/superpowers/plans/2026-07-02-data-expansion-todos.md`. | D1+D2 ~2 days; D7/D8 feasibility ~half day each | **open** — D2 DONE 2026-07-06 (kernel == Stone Soup); D7 NO-GO 2026-07-06 (CC-BY-NC-SA); D8 DONE 2026-07-08 (extraction 487e747); **D10 NO-GO 2026-07-08** (CC BY-NC 4.0, data+API; desk check only, nothing downloaded — see `docs/baselines/2026-07-08_d10_gfw_anchorages.md`): does NOT shorten D3 (watch-circle absent from GFW; dwell/transition are AIS-derived = the veto's existing path) → D3 stays queued against MarineCadastre (US public domain, commercially clean), sharpened to per-vessel watch-circle+dwell mining; KEEP-side anchorage prior redirected to ENC `ACHARE`/`ACHBRT` charted polygons (authoritative, surveyed, license-clean) when the corroboration seam lands — do NOT wire a GFW-derived prior |
