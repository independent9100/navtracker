...
ADR 0001 A2 family) — now the primary path**: no cross-workload sensor
dependency, and the harbor pier becomes a scoreable guard (persistence
alone would re-admit the pier; the probe must find what separates a moving
shore-hugger from persistent structure). Caution: the *unconditional*
land-only A2 floor is already measured-dead (2026-07-02 R1 A/B: philos
card_err +6.9→+40.15, gospa 73.1→106.9) — only a *conditioned* floor is on
the table. (c) accept a lowered gate + re-price the philos regression that
killed the naive version (gospa 73.1→100 — measured 2026-06-30, so (c) is
likely dead; re-measure only if (b) fails).
