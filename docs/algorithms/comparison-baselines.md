...
**Known blocker + candidate paths (ranked, unmeasured).** The no-birth
zone: (a) ADR 0001 A3 sensor-aware suppression (the principled fix parked
2026-06-30 — suppress births only where a sensor that WOULD see the target
says empty; needs camera/AIS corroboration, both of which now exist as
substrate); (b) a conditional coverage floor (floor applies only outside
corroborated near-shore bands); (c) accept a lowered gate + re-price the
philos regression that killed the naive version (gospa 73.1→100 — measured
2026-06-30, so (c) is likely dead; re-measure only if (a)/(b) fail).
