...
| Synthetic geometry breadth bench (Project E, Tasks 1–6) | Cl-2 / Cl-3 | **Measured 2026-06-30.** 5 geometry scenarios + 2 shore-clutter scenarios. PMBM beats MHT on `parallel_lanes_dense` (GOSPA −20%, id_switches 3.4→0, lifetime 0.883→0.975); equivalent on crossing_60/90 and convoy; slight PMBM edge on crossing_30 (GOSPA −7%). No geometry regression. Shore-clutter A/B: land ON collapses open-scenario card_err +29→−0.05 and gospa_false 5811→1 with real targets intact (lifetime 0.975). KNOWN BUG: nearshore scenario — land ON kills the 10m-offshore real target (lifetime 0) due to birth_existence_target=min_new_bernoulli_existence=0.1 + land_scale=0.6 → r_new=0.0625 < gate. Fix: lower min_new_bernoulli_existence or offshore_halfwidth_m. The land model is confirmed valid on perfect-truth synthetic data for targets outside the offshore ramp zone. [Cl-2: geometry breadth] [Cl-3: clutter realism / synthetic validation] | done (bug filed) | **measured; nearshore bug open** |
