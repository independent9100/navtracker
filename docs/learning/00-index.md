...
| 28 | [The bearing-wedge hazard](28-bearing-wedge-hazard.md)                  | Camera-only contact (bearing, no range) must still show as *something*: a wedge along the direction. Half-width = 2σ (camera ⊕ heading) with a floor. Handover by *suppression, not deletion* — recomputed each frame so a crossing vessel can't erase a still-seen contact. |
| 29 | [Fusing trackers: track-to-track fusion and covariance intersection](29-track-to-track-fusion.md) | Inputs are *other trackers' tracks*, sensors often unknown. The shared-newspaper problem (double counting when two trackers share a sensor). Pedigree with a first-class "Unknown". Covariance intersection = "never claim to know more than your most careful friend": safe under any unknown correlation. What CI does *not* fix (a lying-confident source). |
