...
Wire the pieces (`app/example.cpp:39-44`):
...
Build a measurement and process it (`app/example.cpp:90-105`):
...
Drain the results (`app/example.cpp:140-155`):
...
- Construct with **no arguments** — it lazily initialises the datum from the first
  `update(pose)` call (`OwnShipProvider.hpp:57`). A second constructor pins an
  explicit `geo::Datum` if you need one (`OwnShipProvider.hpp:61`).
- It **auto-recenters** when own-ship moves more than the threshold in
  `DatumRecenterPolicy` — default **`recenter_threshold_km{30.0}`**
  (`OwnShipProvider.hpp:49-52`). Set `enable_auto_recenter{false}` to pin the
  datum for a whole run.
...
(`OwnShipProvider.hpp:40-45`) to everyone you register via
`registerDatumSink(...)` (`OwnShipProvider.hpp:86`).

Wire a sink that reprojects your tracks (`app/example.cpp:46-55`):
...
> too (`core/pipeline/MhtTracker.hpp:272`).
...
(`OwnShipProvider.hpp:77-80`). Use `hasDatum()` to check
(`OwnShipProvider.hpp:81`). `poseAtOrBefore(t)` gives the pose the engine will use
for a measurement stamped `t` (`OwnShipProvider.hpp:72`).
...
four kinds (`core/types/Ids.hpp:22`):
...
`canInitiateTrack(model)` (`core/estimation/MeasurementModels.hpp:27`) — it is
...
Use `makeMeasurementFromEnuPosition` (`core/types/MeasurementBuilders.hpp:64`):
...
`applyDefaultsIfEmpty` (below) can fill it (`MeasurementBuilders.cpp:144-146`).
...
- `makeMeasurementFromRelativeBearing(...)`
  (`MeasurementBuilders.hpp:32`) — adds own-ship heading, then projects.
- `makeMeasurementFromTrueBearing(...)`
  (`MeasurementBuilders.hpp:46`) — bearing is already true.

Real call site (`app/example.cpp:118-123`):
...
(`MeasurementBuilders.cpp:63-72`). The NMEA adapters do this for you from their
`heading_std_deg` config; if you call the builders directly, inflate
...
track-birth site in all three trackers (e.g. `core/pipeline/Tracker.cpp:175`,
`core/pmbm/PmbmTracker.cpp:424`). A bearing-only reading that gates to no existing
...
(`core/types/SensorDefaults.hpp:35`):
...
(`SensorDefaults.cpp:65-71`). The `pessimisticSensorDefaults()` values
(`SensorDefaults.cpp:49-63`) are deliberately conservative and cover six
(sensor, model) pairs:
...
Any other (sensor, model) pair is left empty — the tracker will refuse a
measurement with no covariance, so supply your own `R` for those. Operators with
real sensor specs override the relevant `pessimisticSensorDefaults()` fields
before use.

**`covariance_is_default`** is diagnostic only — the tracker behaves identically
whether it is true or false (`Measurement.hpp:48-51`). It flows through to the
...
- **`EoIrAdapter`** (`adapters/eoir/EoIrAdapter.hpp`) takes a `CameraDetection`
  struct (relative bearing + optional range).
...
(`OwnShipNmeaAdapter.hpp:40-59`) carries GPS-noise, velocity-source, and
multi-heading-source knobs. The key one is **`gps_heading_talkers`** (empty by
...
`heading_true_deg` (`OwnShipProvider.hpp:22`). The v3 multi-source fields
(`gps_true_heading_deg`, `magnetic_heading_deg`, `magnetic_variation_deg`) default
to `NaN` = "not present" (`OwnShipProvider.hpp:29-33`).
...
> uncertainty, a perfect gyro (`adapters/arpa/ArpaAdapter.hpp:15`,
> `adapters/eoir/EoIrAdapter.hpp:27`). If your heading comes from a real compass,
...
- **v3** (the three gyro-vs-X kinds) — `OwnShipNmeaAdapter` dispatches these
  ...
  `gps_heading_talkers` (`OwnShipNmeaAdapter.hpp:82`). RMC → COG obs, tagged HDT →
  GPS-heading obs, HDG → magnetic obs.
- **v1** — extract AIS↔ARPA pairs with the free function `extractPairs(...)`
  (`core/bias/AisArpaPairExtractor.hpp:27`) and feed them to `observe(...)`.
- **v2** — `Tracker::setBearingInnovationSink(&est)`
  (`core/pipeline/Tracker.hpp:36`); the tracker emits a `BearingInnovation` after
  each bearing update.

Wiring (`tests/integration/test_full_stack_pipeline.cpp:159-170`):
...
bias above. Wire it only if you have mounting misalignment to calibrate; details in
`docs/algorithms/`.
...
(`core/output/TrackOutput.hpp:78`). The `TrackOutput` (`TrackOutput.hpp:49-58`)
carries:
...
- `velocity` — `sog_m_per_s`, `cog_deg` (true, [0,360)), their σ, and `is_valid`.
  ...
  velocity covariance (`TrackOutput.cpp:101-107`).
...
`TrackStatus` (`core/types/Ids.hpp:19`) has **four** values: `Tentative`,
...
- **`ITrackSink`** (`ports/ITrackSink.hpp`) — `onTrackInitiated`,
  ...
  `TrackManager::setTrackSink(...)` (`core/tracking/TrackManager.hpp:37`). (PMBM
  has its own `setTrackSink` at `core/pmbm/PmbmTracker.hpp:640`.)
...
hysteresis. `CpaEvaluatorConfig` (`CpaEvaluator.hpp:16`) defaults:
...
(`core/collision/Cpa.hpp:81`) carries CPA distance/σ, TCPA/σ, and
...
(`StaticHazardEvaluator.hpp:15`) defaults: `exit_hysteresis = 1.1`,
`emit_updates = false`. Convert a `StaticObstacle` to an operator-facing
`StaticHazardOutput` with `toStaticHazardOutput(obs)`
(`core/output/StaticHazardOutput.hpp:40`). See §7 for the obstacle input.
...
Build a `std::vector<StaticObstacle>` (`core/types/StaticObstacle.hpp:27`) from
...
(`core/static/StaticObstacleModel.hpp:36`):
...
(`adapters/land/GeoJsonCoastline.hpp:16`); tune the ramp with
`CoastlinePriorParams` (`core/land/CoastlineGeometry.hpp:13`,
...
- `MhtTracker::Config` (`core/pipeline/MhtTracker.hpp:48`) — e.g.
...
- `PmbmTracker::Config` (`core/pmbm/PmbmTracker.hpp:47`) — e.g.
...
- **Empty covariance never defaulted** — the tracker refuses a measurement with no
  `R`. Call `applyDefaultsIfEmpty` or supply your own. → §3.
...
|---|---|---|---|
| `OwnShipNmeaAdapterConfig` | `adapters/own_ship/OwnShipNmeaAdapter.hpp:40` | Own-ship NMEA parsing: GPS noise, velocity source, heading sources | `uere_m{5.0}`, `gps_heading_talkers{}`, `prefer_rmc_velocity{true}` |
| `ArpaAdapterConfig` | `adapters/arpa/ArpaAdapter.hpp:14` | Radar TTM/TLL → measurements | `heading_std_deg{0.0}`, `position_std_m{50.0}`, `bearing_std_deg{1.0}` |
| `EoIrAdapterConfig` | `adapters/eoir/EoIrAdapter.hpp:26` | EO/IR camera heading σ | `heading_std_deg{0.0}` |
| `CpaEvaluatorConfig` | `core/collision/CpaEvaluator.hpp:16` | Collision-risk thresholds + hysteresis | `d_threshold_m{500.0}`, `enter_probability{0.5}`, `exit_probability{0.3}` |
| `StaticHazardEvaluatorConfig` | `core/collision/StaticHazardEvaluator.hpp:15` | Keep-clear-ring alerts | `exit_hysteresis{1.1}`, `emit_updates{false}` |
| `HeadingBiasEstimatorConfig` | `core/bias/HeadingBiasEstimator.hpp:14` | Gyro-bias KF tuning + per-kind gates | `initial_variance_rad2` (5°)², `cog_min_sog_mps{3.0}`, `bi_min_range_m{50.0}` |
| `AisArpaPairExtractorConfig` | `core/bias/AisArpaPairExtractor.hpp:11` | AIS↔ARPA v1 pairing window | `cycle_window_seconds{0.5}`, `ais_position_std_fallback_m{10.0}` |
| `SensorBiasEstimatorConfig` | `core/bias/SensorBiasEstimator.hpp:13` | Per-sensor mounting-bias KF | `min_range_m{50.0}`, `max_time_skew_seconds{1.0}`, `outlier_sigma{5.0}` |
| `SensorBiasPairExtractorConfig` | `core/bias/SensorBiasPairExtractor.hpp:12` | Per-sensor bias pairing window | `cycle_window_seconds{0.5}`, `sensor_position_std_fallback_m{10.0}` |
| `CrossSensorEligibilityConfig` | `core/bias/SensorBiasPairExtractor.hpp:28` | Which cross-sensor pairs are trustworthy | `min_existence_probability{0.95}`, `max_position_cov_trace_m2{25.0}` |
| `OwnShipVelocityEstimatorConfig` | `core/own_ship/OwnShipVelocityEstimator.hpp:12` | Own-ship velocity smoothing (nested in NMEA config) | `window_size{8}`, `maneuver_dv_threshold_mps{0.5}` |
| `UereEstimatorConfig` | `core/own_ship/UereEstimator.hpp:10` | Adaptive GPS-σ estimation (nested in NMEA config) | `window_size{8}`, `min_sigma_m{0.05}` |

Two big tracker `Config` structs are covered in §8 rather than tabulated here
because of their size: `MhtTracker::Config` (`core/pipeline/MhtTracker.hpp:48`) and
`pmbm::PmbmTracker::Config` (`core/pmbm/PmbmTracker.hpp:47`).

Non-`Config` tuning structs a consumer also touches: `DatumRecenterPolicy`
(`core/own_ship/OwnShipProvider.hpp:49`, §2), `StaticObstacleParams`
(`core/static/StaticObstacleModel.hpp:16`, §7), `CoastlinePriorParams`
(`core/land/CoastlineGeometry.hpp:13`, §7), `LiveOccupancyParams`
(`core/static/LiveOccupancyModel.hpp:20`, §7).
