...
void OwnShipEmitter::emit(const EmitContext& ctx) {
  ...
  while (next_emit_ <= ctx.now) {
    ...
    // of snprintf %f.
    {
      ...
      double abs_hdg = hdg < 0.0 ? -hdg : hdg;
      long long milli = static_cast<long long>(std::llround(abs_hdg * 1000.0));
      const int int_part = static_cast<int>(milli / 1000LL);
      const int frac_part = static_cast<int>(milli % 1000LL);
      char buf[48];
      std::snprintf(buf, sizeof(buf), "GPHDT,%s%d.%03d,T",
                    hdg < 0.0 ? "-" : "", int_part, frac_part);
