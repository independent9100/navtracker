...
class ITrackSink {
 public:
  ...
  /** Track just created in Tentative status. */
  virtual void onTrackInitiated(const TrackLifecycleEvent& e) = 0;
  /** Track promoted Tentative -> Confirmed (fires once). */
  virtual void onTrackConfirmed(const TrackLifecycleEvent& e) = 0;
  /** Track state folded a measurement in this cycle (any status). */
  virtual void onTrackUpdated(const TrackLifecycleEvent& e) = 0;
  /** Track about to be erased (status is the pre-erasure value). */
