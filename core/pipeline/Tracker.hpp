...
class Tracker {
 public:
  /**
   * Wire the pipeline to its swappable strategies: `estimator` (per-track
   * predict/update), `associator` (measurement-to-track assignment), the
   * `manager` that owns the track set, and the `miss_timeout_seconds`
   * policy for ageing unobserved tracks.
   */
  ...
  /** Process a single measurement through the predict-associate-update/initiate cycle. */
  ...
  /** Count of measurements dropped by the stale-input guard. */
