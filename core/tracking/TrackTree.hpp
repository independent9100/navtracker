...
class TrackTree {
 public:
  /** Construct a tree seeded from `root`, tagged with a stable `external_id`. */
  TrackTree(TrackId external_id, const TrackTreeNode& root);

  /** Stable external identity of this tree (never reused after deletion). */
  TrackId externalId() const { return external_id_; }
  /** Read-only view of every node (indexable by node_id). */
  const std::vector<TrackTreeNode>& nodes() const { return nodes_; }
  /** Mutable view of every node (used by MhtTracker to set protection flags etc.). */
  std::vector<TrackTreeNode>& mutableNodes() { return nodes_; }

  /** Indices of the current leaves (nodes with is_leaf == true). */
  std::vector<std::size_t> leafIndices() const;
  /** Index of the highest-scoring current leaf. */
  ...
  /** Record the protected-alternative count from the last global solve. */
  ...
  /** Set the IPDA confirmation-hysteresis memory flag. */
