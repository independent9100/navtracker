...
/**
 * One constant-velocity target sampled at `sample_times_seconds`, emitting a
 * Position2D measurement per tick with isotropic Gaussian noise. The baseline
 * single-target scenario for CV filter / lifecycle tests.
 */
...
/**
 * Two constant-velocity targets sharing `velocity`, starting at `start_a` and
 * `start_b`. Position2D measurements per tick with isotropic noise. Truth ids
 * are 1 and 2. Exercises two-target association with no crossing.
 */
